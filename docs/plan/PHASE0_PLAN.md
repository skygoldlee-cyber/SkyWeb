...
PWA(Progressive Web App)는 SkyWeb의 **앱 설치 경험과 빠른 재방문**을 위해 도입합니다. 분석은 온라인+워커가 필요하므로 "오프라인 분석"은 제공하지 않으며, 오프라인은 앱 셸·안내 페이지로 한정합니다. Phase 0에서는 개념만 학습합니다.

### PWA 핵심 개념
- **Service Worker**: 브라우저 백그라운드 스크립트 (캐싱, 오프라인 셸). 도구는 **Serwist**(next-pwa 후속, Next.js 공식 권장) 사용
- **manifest**: 앱 메타데이터 (이름, 아이콘 — maskable 포함, 테마 색상)
- **HTTPS 필수**: PWA는 HTTPS(또는 localhost)에서만 동작
- **설치 가능성**: `beforeinstallprompt`(Android/Chromium). iOS Safari는 미지원 → 수동 "홈 화면에 추가" 안내 필요

### PWA 필요성 (SkyWeb 맥락)
- 네이티브 앱과 유사한 설치 경험 (앱스토어 승인 없이) + 홈스크린 재방문
- 앱 셸 캐싱으로 재진입·로딩 개선
- 오프라인 시 안내 페이지 표시 (분석 결과·이미지는 **오프라인 저장·표시하지 않음** — PIPA 민감정보)
- (v2) 푸시 알림으로 재참여 — Web Push 필요, iOS는 설치 PWA+16.4↑ 한정이라 v2로 분리

### 학습 체크리스트 (PWA)
- [ ] Service Worker 기본 개념 + Serwist 역할 이해
- [ ] manifest 구조 이해 (maskable 아이콘, viewport themeColor)
- [ ] PWA 설치 흐름 이해 (`beforeinstallprompt` + iOS 수동 안내)
- [ ] 캐싱-보안 원칙 이해: 민감 API·이미지는 `NetworkOnly`(캐시 금지)
- [ ] 검증은 DevTools Application 패널로 (Lighthouse PWA 점수는 v12에서 폐지됨)

> 실제 구현은 Phase 4(Serwist 기본 설정)부터 Phase 8(검증·보안 회귀)까지 단계적으로 진행합니다.
...
**PWA**: Service Worker(**Serwist**) · manifest(maskable) · 설치 흐름(+iOS 안내) · 캐싱-보안(NetworkOnly) · DevTools Application 검증
