...
SkyWeb은 피부분석 서비스로 다음과 같은 이유로 PWA 도입이 필요합니다:

- **오프라인 접근성**: 분석 결과 및 히스토리를 오프라인에서도 조회 가능
- **앱 설치 경험**: 네이티브 앱과 유사한 설치/실행 경험 제공 (앱스토어 승인 없이)
- **푸시 알림**: 분석 완료, 추천 솔루션 업데이트 등 실시간 알림
- **성능 향상**: 서비스 워커 캐싱으로 로딩 속도 개선
- **사용자 유지**: 홈스크린 아이콘으로 재방문 유도

### 단계별 검증 로직

PWA 구현 시 다음 단계별로 검증합니다:
...
```json
{
  "name": "SkyWeb - AI 피부분석",
  "short_name": "SkyWeb",
  "description": "AI 기반 피부 상태 분석 서비스",
  "start_url": "/",
  "display": "standalone",
...
192x192, 512x512 크기의 PNG 아이콘을 `public/` 디렉토리에 배치
